...
from basicsr.models.archs.Baseline_arch import Baseline, BaselineLocal, BaselineBlock
...
class RestoreNet(nn.Module):
    ...
    def __init__(self, img_channel=3, width_deblur=16, middle_blk_num_deblur=1, enc_blk_nums_deblur=[], dec_blk_nums_deblur=[], \
        ...
        self.intro_supple = nn.Conv2d(in_channels=img_channel * 2, out_channels=width, kernel_size=3, padding=1, stride=1, groups=1,
                              bias=True)
        self.ending_supple = nn.Conv2d(in_channels=width, out_channels=img_channel, kernel_size=3, padding=1, stride=1, groups=1,
                              bias=True)

        self.encoders_supple = nn.ModuleList()
        self.decoders_supple = nn.ModuleList()
        self.middle_blks_supple = nn.ModuleList()
        self.ups_supple = nn.ModuleList()
        self.downs_supple = nn.ModuleList()

        chan = width
        for num in enc_blk_nums_denoise:
            self.encoders_supple.append(
                nn.Sequential(
                    *[BaselineBlock(chan, dw_expand, ffn_expand_deblur) for _ in range(num)]
                )
            )
            self.downs_supple.append(
                nn.Conv2d(chan, 2*chan, 2, 2)
            )
            chan = chan * 2

        self.middle_blks_supple = \
            nn.Sequential(
                *[BaselineBlock(chan, dw_expand, ffn_expand_deblur) for _ in range(middle_blk_num_deblur)]
            )

        for num in dec_blk_nums_deblur:
            self.ups_supple.append(
                nn.Sequential(
    ...
    def forward(self, inp):
        ...
        x1 = self.deblur(inp)
        x1 = inp - x1 # Residual r1

        x2 = self.denoise(inp)
        x2 = inp - x2 # Residual r2
        
        x = torch.cat((x1, x2), 1)

        x = self.intro_supple(x)

        encs = []

        for encoder, down in zip(self.encoders_supple, self.downs_supple):
            x = encoder(x)
            encs.append(x)
            x = down(x)

        x = self.middle_blks_supple(x)

        for decoder, up, enc_skip in zip(self.decoders_supple, self.ups_supple, encs[::-1]):
            x = up(x)
            x = x + enc_skip
            x = decoder(x)

        x = self.ending_supple(x)
        x = inp + x
        return x[:, :, :H, :W]
...
class RestoreNetLocal(Local_Base, RestoreNet):
    def __init__(self, *args, train_size=(1, 3, 256, 256), fast_imp=False, **kwargs):
        Local_Base.__init__(self)
        RestoreNet.__init__(self, *args, **kwargs)
...
if __name__ == '__main__':
    ...
    net = RestoreNet(img_channel=img_channel, width=width, middle_blk_num=middle_blk_num,
