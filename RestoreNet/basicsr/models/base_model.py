...
class BaseModel():
    ...
    def load_network(self, net, load_path, strict=True, param_key='params'):
        ...
        """
        net = self.get_bare_model(net)
        if net.__class__.__name__ != "StraightNetLocal":
            #logger.info(
        ...
        if net.__class__.__name__ == "RestoreNetLocal" or net.__class__.__name__ == "StraightNetLocal":

            #Load Deblur
            load_deblur = torch.load(
                "./experiments/pretrained_models/Baseline-GoPro-width64.pth", map_location=lambda storage, loc: storage)
            if param_key is not None:
                load_deblur = load_deblur[param_key]
            #print(' load net keys', load_deblur.keys)
            # remove unnecessary 'module.'
            for k, v in deepcopy(load_deblur).items():
                if k.startswith('module.'):
                    load_deblur[k[7:]] = v
                    load_deblur.pop(k)
            self._print_different_keys_loading(net.deblur, load_deblur, strict)
            net.deblur.load_state_dict(load_deblur, strict=strict)
        
            #Load Denoise
            load_denoise = torch.load(
                "./experiments/pretrained_models/Baseline-SIDD-width64.pth", map_location=lambda storage, loc: storage)
            if param_key is not None:
                load_denoise = load_denoise[param_key]
            #print(' load net keys', load_denoise.keys)
            # remove unnecessary 'module.'
            for k, v in deepcopy(load_denoise).items():
                if k.startswith('module.'):
                    load_denoise[k[7:]] = v
                    load_denoise.pop(k)
            self._print_different_keys_loading(net.denoise, load_denoise, strict)
            net.denoise.load_state_dict(load_denoise, strict=strict)

            net.deblur.requires_grad_(False)
            net.denoise.requires_grad_(False)
